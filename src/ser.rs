...
impl ser::SerializeMap for MapSerializer<'_> {
    ...
    fn end(self) -> std::result::Result<Self::Ok, Self::Error> {
        let mut aux_map_encoder = WriteEncoder::new(&[], Context::new());
        aux_map_encoder.write_map_length(&self.map_entries)?;
        aux_map_encoder.write(&self.map_serializer.write_encoder.get_buffer())?;
...
use crate::to_vec;
  ...
  #[test]
  fn test_write_bytes() {
      let cases = [Case::new("Bytes", serde_bytes::ByteBuf::from([1]), &[196, 1, 1])];
